...
train_tbl <- training(casas_split)
test_tbl <- testing(casas_split)
```
...
```{r}
train_tbl %>% group_by(SaleCondition) %>% 
  count()
```
...
Requerimos también calcular la variable adicional de *Pisos*. Los datos sólo reportan
...
función que aplicaremos tanto a entrenamiento como validación. 
Construimos entonces nuestra cadena de procesamiento:

```{r}
limpiar_casas <- function(dat){
    dat_nueva <- dat %>% 
      ...
      filter(SaleCondition %in% c("Normal", "Famliy", "AdjLand")) %>% 
      select(Id, tiene_piso_2:calidad)
    dat_nueva
}
train_tbl <- limpiar_casas(train_tbl)
test_tbl <- limpiar_casas(test_tbl)
```
...
```{r}
graf_ent <- ggplot(train_tbl, aes(x = calidad, y = precio_m2_miles)) + 
...
Ahora podemos ajustar al conjunto de entrenamiento_

```{r}
casas_receta <- recipe(precio_m2_miles ~ ., data = train_tbl) %>%
  step_rm(precio_miles, habitable_m2) %>%
  update_role(Id, new_role = "ID") %>% 
  step_center(all_predictors())
casas_receta$var_info
```
...
```{r}
ajuste <- casas_wflow %>% fit(data = train_tbl)
ajuste 
```

Ahora podemos comparar ajustados con observados (el ajuste se hizo con 
mínimos cuadrados):
