...
```{r, message = FALSE}
modelo_casas <- lm(precio_m2_miles ~ calidad + tiene_piso_2, data = train_tbl)
preds_val <- predict(modelo_casas, newdata = test_tbl, interval = "prediction",
                     level = 0.95)
preds_val_precio <- preds_val * test_tbl$habitable_m2
valida_g <- bind_cols(preds_val_precio %>% as_tibble, test_tbl %>% select(precio_miles))
head(valida_g)
...
Y como vemos, la calibración es aceptable.
