...
library(tidymodels)
set.seed(1213)
datos_split <- initial_split(datos, prop = 0.67)
...
vmc_15 <- nearest_neighbor(neighbors = 15) %>% 
  set_engine("kknn") %>% 
  set_mode("regression")
rend_vmc <- fit(vmc_15, rendimiento_kpl ~ peso_kg, datos_entrena)
```

```{r}
dat_graf <- tibble(peso_kg = seq(700, 2200, by = 10))
dat_graf <- dat_graf %>% 
  mutate(pred_15 = predict(rend_vmc, dat_graf) %>% pull(.pred))
ggplot(datos_entrena, aes(x = peso_kg)) +
  geom_point(aes(y = rendimiento_kpl), alpha = 0.6) + 
  geom_line(data = dat_graf, aes(y = pred_15), col = 'red', size = 1.2)
```
...
```{r}
vmc_3 <- nearest_neighbor(neighbors = 3) %>% 
  set_engine("kknn") %>% 
  set_mode("regression")
rend_vmc_3 <- fit(vmc_3, rendimiento_kpl ~ peso_kg, datos_entrena)
dat_graf <- dat_graf %>% 
  mutate(pred_5 = predict(rend_vmc_3, dat_graf) %>% pull(.pred))
ggplot(datos_entrena, aes(x = peso_kg)) +
  geom_point(aes(y = rendimiento_kpl), alpha = 0.6) + 
  geom_line(data = dat_graf, aes(y = pred_5), col = 'red', size = 1.2)
```

En nuestro caso, los errores de prueba son

```{r}
eval_1 <- predict(rend_vmc, datos_prueba) %>% 
  bind_cols(datos_prueba) %>% 
  rmse(rendimiento_kpl, .pred) %>% 
    mutate(n_vecinos = 15) 
eval_2 <- predict(rend_vmc_3, datos_prueba) %>% 
  bind_cols(datos_prueba) %>% 
  rmse(rendimiento_kpl, .pred) %>% 
    mutate(n_vecinos = 3) 
