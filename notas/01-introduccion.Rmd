...
y queremos predecir, para cada juego de entradas dada, qué valor de
...
que influyen sobre $y$. Estas variables pueden ser modeladas de forma probabilista.

Nuestro primer propósito es construir una función $f$ tal que si observamos
cualquier $(x_1, x_2, \ldots, x_n) \to y$, entonces

$$y_f = f(x_1, x_2, \ldots, x_n) \approx y$$
es decir, con una regla o algoritmo $f$ podemos _predecir con buena precisión_ 
...
y el nuestra predicción. Pero quizá nos interese más el error relativo:

$$L \left (f(\mathbf{x}^{(i)}), \mathbf{y}^{(i)}\right ) = \frac{\left |\mathbf{y}^{(i)} - f(\mathbf{x}^{(i)})\right |}{f(\mathbf{x}^{(i)})}$$
...
que nuestra regla $f$ es un mal predictor. Una idea simple es resumir evaluando
el error promedio sobre los datos de prueba. El **error de prueba** de $f$
sobre los datos es 

$$ \widehat{Err} = \frac{1}{m} \sum_{j=1}^m L(\mathbf{y}^{(i)} , f(\mathbf{x}^{(i)}))2$$
Por ejemplo, si usamos la pérdida absoluta, 

$$ \widehat{Err} = \frac{1}{m} \sum_{j=1}^m |\mathbf{y}^{(i)} - f(\mathbf{x}^{(i)})|$$
o la cuadrática:

$$ \widehat{Err} = \frac{1}{m} \sum_{j=1}^m (\mathbf{y}^{(i)} - f(\mathbf{x}^{(i)})^2$$
...
En el caso donde tenemosel conjunto de datos de muestra es una muestra iid del proceso
generador de datos, esta cantidad estima la pérdida esperada

$$E_\pi(L(y, f(x)))$$
donde el valor esperado es sobre la distribución conjunta de $(x, y) ~ \pi$. Nuestro
propósito es minimizar esta pérdida esperada.
